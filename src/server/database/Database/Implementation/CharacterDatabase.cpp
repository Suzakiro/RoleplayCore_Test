...
CharacterDatabaseConnection::~CharacterDatabaseConnection()
{
    ...
    PrepareStatement(CHAR_INS_HOUSING_DECORATION,
        ...
        "(id, house_id, item_id, pos_x, pos_y, pos_z, rot_x, rot_y, rot_z, rot_w, scale, type, is_locked) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_HOUSING_FIXTURE,
        "INSERT INTO housing_fixtures (id, house_id, type, fixture_id, slot) "
        "VALUES (?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_HOUSING_ROOM,
        ...
        "(id, house_id, room_index, pos_x, pos_y, pos_z, orientation, size_x, size_y, "
        " ceiling_type, door_type, component_theme) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_HOUSING_ROOM,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_HOUSING_MAX_DECOR_ID,
        "SELECT COALESCE(MAX(id), 0) FROM housing_decorations",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_HOUSING_MAX_FIXTURE_ID,
        "SELECT COALESCE(MAX(id), 0) FROM housing_fixtures",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_HOUSING_MAX_ROOM_ID,
        "SELECT COALESCE(MAX(id), 0) FROM housing_rooms",
        CONNECTION_SYNCH);
}

...
{
	uint32 activeProducts = 0;
    if (QueryResult countResult = LoginDatabase.PQuery("SELECT COUNT(*) FROM battlepay_product WHERE storefront_id = {} AND active = 1", packet.StoreFrontID))
        activeProducts = countResult->Fetch()[0].GetUInt32();

    TC_LOG_INFO("network", "BattlePay storefront request from account {} storefront {} activeProducts {}", GetAccountId(), packet.StoreFrontID, activeProducts);
